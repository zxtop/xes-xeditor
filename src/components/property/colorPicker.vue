...
export default {
  ...
  methods: {
    ...
    changeColor(value,ev){
      this.clickBlur(ev)
      let getColor = this.getStyleValue(this.objs,'fill') 
      this.colo = value;
      this.$refs.html5Color.value = value;
      this.styleObjF.background = value;
      let key = "fill"
      if(this.objs.type == 7){
        this.$store.commit("SET_SELECT_OBJ_VALUE", {arr:[this.objs],key:key,value:value,isStack:{modifyState:true,startContent:getColor}});
      }else if(this.objs.type == 3){
        this.$store.commit("SET_SELECT_OBJ_VALUE", {arr:[this.objs],key:key,value:value,isStack:{modifyState:true,startContent:getColor}});
      }
    },
    updataValue(value) {
  ...
  mounted() {
    // 点击页面上其他地方，关闭弹窗
    this.styleObjF.background = this.getStyleValue(this.objs,'fill');
    document.onclick = e => {
      this.openStatus = false;
    };
  }
